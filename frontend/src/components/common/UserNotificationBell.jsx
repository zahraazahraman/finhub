...
export default function UserNotificationBell() {
  ...
  const { unreadCount, setUnreadCount, refreshCount } = useUserNotifications();
  ...
  const fetchRecent = async () => {
    setLoading(true);
    const result = await MyNotificationsBLL.getRecent();
  ...
  return (
    <>
      ...
      {open && ReactDOM.createPortal(
        <div
          style={{ position: "fixed", top: position.top, left: position.left, zIndex: 1000, boxShadow: "var(--shadow-lg)" }}
