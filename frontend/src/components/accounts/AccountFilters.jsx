...
export default function AccountFilters({
  ...
  const showingLabel = useMemo(() => {
    if (totalCount === 0) return "No accounts yet";
  ...
  return (
    <div className="mb-6 rounded-xl border border-skin-border bg-skin-secondary/40 p-3 sm:p-4 animate-fade-in">
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_14rem_14rem] lg:items-end">
        <div>
          <label htmlFor="account-search" className="block text-xs font-medium text-skin-text-muted mb-1.5">
            Search
          </label>
        ...
        <div>
          <label htmlFor="account-type-filter" className="block text-xs font-medium text-skin-text-muted mb-1.5">
            Account Type
          </label>
          <select
        ...
        <div>
          <label htmlFor="account-balance-state-filter" className="block text-xs font-medium text-skin-text-muted mb-1.5">
            Balance State
          </label>
          <select
            id="account-balance-state-filter"
            value={balanceStateFilter}
            onChange={(e) => onBalanceStateFilterChange(e.target.value)}
            className="w-full rounded-lg border border-skin-border bg-skin-bg px-3 py-2 text-sm text-skin-text focus:border-emerald-500 focus:outline-none"
          >
            {BALANCE_STATE_OPTIONS.map((option) => (
              <option key={option.value} value={option.value}>
                {option.label}
              </option>
            ))}
