...
function ChatBubble({ message }) {
  ...
  return (
    <div className={`flex items-end gap-2.5 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
      ...
      {/* AI avatar — bg-skin-brand-subtle circle, brand-colored icon */}
      {!isUser && (
    ...
    >
      ...
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-skin-border bg-skin-card flex-shrink-0">
        <div className="flex items-center gap-2.5">
          {/* Status dot — uses brand color, which is green in both modes */}
          <div className="w-2 h-2 rounded-full bg-brand" />
          <span className="text-sm font-medium text-skin-text">AI Financial Consultant</span>
        </div>
      ...
      {/* ── Error banner — uses standard Tailwind red-* (danger-* is not defined) ── */}
      {error && (
      ...
      <div className="px-4 py-3.5 border-t border-skin-border bg-skin-card flex-shrink-0">
        {/*
          Focus ring: uses CSS variable directly as an arbitrary value.
          bg-brand/40 is NOT used because opacity modifiers break with CSS variable colors.
          Instead: disabled uses opacity-50 on a solid bg-brand.
        */}
