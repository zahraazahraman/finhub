...
function StarRating({ rating }) {
  ...
  return (
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map((s) => (
        <svg key={s} className={`w-3.5 h-3.5 ${s <= Math.round(r) ? "text-yellow-400" : "text-slate-700"}`} fill="currentColor" viewBox="0 0 20 20">
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
        ...
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-semibold text-sm px-4 py-2 rounded-xl transition-all"
            >
              Get Started
            </button>
          </div>
      ...
      <section className="relative pt-32 pb-24 px-6 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          <p className="text-slate-400 text-lg leading-relaxed max-w-2xl mx-auto mb-10">
            FinHub gives you full control over your finances — track spending, set goals, manage investments, and get AI-powered insights, all in one place.
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button
              onClick={() => navigate("/register")}
              className="w-full sm:w-auto flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-semibold px-8 py-3.5 rounded-xl transition-all text-sm"
            >
              Start for free
            ...
            <button
              onClick={() => navigate("/login")}
              className="w-full sm:w-auto flex items-center justify-center gap-2 border border-slate-700 hover:border-slate-500 text-slate-300 hover:text-white font-medium px-8 py-3.5 rounded-xl transition-all text-sm"
            >
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-400 text-lg max-w-2xl mx-auto">
              From tracking daily expenses to AI-powered investment analysis — FinHub has it all.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {steps.map((step, i) => (
              <div key={step.number} className="relative text-center">
                {/* Connector line */}
                {i < steps.length - 1 && (
                  <div className="hidden md:block absolute top-8 left-1/2 w-full h-px bg-slate-800" />
                )}
      ...
      {/* ── Consultants ── */}
      <section id="consultants" className="py-24 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
            ...
            <p className="text-slate-400 text-lg max-w-2xl mx-auto">
              Get personalized advice from verified experts or let our AI guide you through your financial journey.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            {consultants.length > 0 ? consultants.map((c) => (
              <div key={c.consultant_id} className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 hover:border-slate-700 transition-all">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    <p className="text-white font-semibold">{c.first_name} {c.last_name}</p>
                    <p className="text-slate-500 text-xs">{c.email}</p>
                  </div>
                </div>
                <span className="inline-block bg-slate-800 text-slate-300 text-xs px-3 py-1 rounded-full border border-slate-700 mb-3">
                  {c.specialization}
                </span>
          ...
          <div className="text-center">
            <button
              onClick={() => navigate("/register")}
              className="border border-slate-700 hover:border-emerald-500/50 text-slate-400 hover:text-emerald-400 text-sm px-6 py-2.5 rounded-xl transition-all"
            >
        ...
        <div className="max-w-4xl mx-auto">
          <div className="relative bg-emerald-500/5 border border-emerald-500/20 rounded-3xl p-12 text-center overflow-hidden">
            ...
            <div className="relative">
              ...
              <button
                onClick={() => navigate("/register")}
                className="bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-semibold px-8 py-3.5 rounded-xl transition-all text-sm inline-flex items-center gap-2"
              >
      ...
      <footer className="border-t border-slate-800 py-8 px-6">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-emerald-400">Hub</span>
            <span className="text-slate-600 text-sm ml-2">· See where your money goes. Know where it grows.</span>
          </div>
